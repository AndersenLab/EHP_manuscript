...
#------------------------------------------------------------------------------#
# Test new orthogonal regression
#------------------------------------------------------------------------------#
v <- prcomp(cbind(log10(noCarb_plot_dat$gm_mean_x),log10(noCarb_plot_dat$gm_mean_y)))$rotation
beta <- v[2,1]/v[1,1]
intercept <- mean(log10(noCarb_plot_dat$gm_mean_y)) - beta*mean(log10(noCarb_plot_dat$gm_mean_x))
#------------------------------------------------------------------------------#
...
# save the figure
cowplot::ggsave2(fig2, filename = glue::glue("figures/{today}_figure2.png"), width = 4.5, height = 4.5)

# save the data for ploting and the regression coefficients
rio::export(plot_dat, file = glue::glue("data/processed/{today}_fig2.plot.data.csv"))
rio::export(noCarb_orthreg_df, file = glue::glue)"data/processed/{today}_fig2.orth.reg.csv"))
