...
#------------------------------------------------------------------------------#
# Part 1: Get ToxCast data that match the nematode toxicants and apply
# simple filters.
#------------------------------------------------------------------------------#
...
#------------------------------------------------------------------------------#
# Part 2: Get Zebrafish data from ToxCast, fuse with SG dataset and perform
# orthogonal regression
#------------------------------------------------------------------------------#
# filter assays to desired zebrafish assays then join mc5 data
zf1 <- assays %>%
  dplyr::filter(organism == "zebrafish" & assay_design_type_sub %in% c("embryo development","embryonic mortality")) %>%
  dplyr::left_join(., mc5) %>%
  dplyr::filter(casn %in% cas_numbers) %>% # filter to worm cas
  dplyr::left_join(mw_data, by = c("casn" = "CASRN")) %>% # join MW data
  mutate(AC50_mg_L = (ac50 / 1e6) * AVERAGE_MASS * 1000, # Convert µM to mg/L {orig. ac50 * MW * 1000} yikes!
...
#------------------------------------------------------------------------------#
# Part 3: Run orthognal regression with Toxcast zebrafish data
#------------------------------------------------------------------------------#
# load cleaned data 
...
# Setup the zebrafish TOXCAST data to work with the plotting functions
zf1.join <- zf1 %>%
  dplyr::mutate(casn = as.integer(stringr::str_replace_all(casn, pattern = "-", replacement = "")),
                group = ifelse(aeid == 1372, "TOXCAST_zebrafish_Tanguay", "TOXCAST_zebrafish_Padilla"),
  ...
  dplyr::select(cas, chem_name, group, latin_name, strain, test_type,
                test_statistic, duration_d, effect_value, effect_unit, source, endpoint, QC)

# Bind them together
zf.bind <- dplyr::bind_rows(dat, zf1.join)

# look at the distributions of AC50s
effect_hist <- ggplot(zf.bind) +
  aes(x = effect_value, fill = QC) +
  geom_histogram() +
...
# Setup to perform orthogonal regression between nematode and two zf datasets
zf.tanguay.df <- zf.bind.qc %>%
  dplyr::filter(group == "NEMATODE" | group == "TOXCAST_zebrafish_Tanguay")
zf.padilla.df <- zf.bind.qc %>%
  dplyr::filter(group == "NEMATODE" | group == "TOXCAST_zebrafish_Padilla")

# run the orth regressions across all pairs that pass QC
or.zf.tanguay <- pwOrthReg(data = zf.tanguay.df, group = "group", limit.comp = "NEMATODE", min.n = 5, message = T, plot = T)
or.zf.padilla <- pwOrthReg(data = zf.padilla.df, group = "group", limit.comp = "NEMATODE", min.n = 5, message = T, plot = T)
or.zf.padilla.df <- data.table::rbindlist(or.zf.padilla$orthregs)
or.zf.padilla$plots

# run the orth regression across all pairs pass or faill QC
# Setup to perform orthogonal regression between nematode and two zf datasets
zf.tanguay.df.all <- zf.bind %>%
  dplyr::filter(group == "NEMATODE" | group == "TOXCAST_zebrafish_Tanguay")
zf.padilla.df.all <- zf.bind %>%
  dplyr::filter(group == "NEMATODE" | group == "TOXCAST_zebrafish_Padilla")

# run the orth regressions across all pairs that pass QC
or.zf.tanguay.all <- pwOrthReg(data = zf.tanguay.df.all, group = "group", limit.comp = "NEMATODE", min.n = 5, message = T, plot = T)
or.zf.padilla.all <- pwOrthReg(data = zf.padilla.df.all, group = "group", limit.comp = "NEMATODE", min.n = 5, message = T, plot = T)
or.zf.padilla.df.all <- data.table::rbindlist(or.zf.padilla.all$orthregs)
or.zf.padilla.all$plots

# show example of QC filtering for ToxCast data
zf.QC.fitler.eff.plot <- cowplot::plot_grid(or.zf.padilla.all$plots[[1]] + labs(title = "NO Quality Control"), or.zf.padilla$plots[[1]] + labs(title = "With Quality Control"), labels = c("a", "b"), vjust = 1)
zf.QC.fitler.eff.plot
cowplot::ggsave2(zf.QC.fitler.eff.plot, filename = "plots/toxcast_zebrafish_padilla_qc_effect.png", width = 10, height = 5)

#------------------------------------------------------------------------------#
# Part 4: Compare Toxcast zebrafish data to SG zebrafish data
#------------------------------------------------------------------------------#
zf.padilla.df2 <- zf.bind.qc %>%
  dplyr::filter(group == "ZF_EMBRYO2" | group == "TOXCAST_zebrafish_Padilla")
...
or.zf.padilla3$plots

# show the comparison vs SG curated data
zf_comps <- cowplot::plot_grid(or.zf.padilla2$plots[[1]] + labs(title = "Scholz et al. 2016 vs toxcast Padilla ZF embryo"),
                               or.zf.padilla3$plots[[1]] + labs(title = "Padilla et al. 2012 vs toxcast Padilla ZF embryo"),
                               labels = c("a", "b"), vjust = 1)
zf_comps
cowplot::ggsave2(zf_comps, filename = "plots/toxcast_zebrafish_source_comps.png", width = 10, height = 5)

#------------------------------------------------------------------------------#
# Part 5: Testing deming regression inside of orthReg function
#------------------------------------------------------------------------------#
undebug(orthReg)
or.zf.padilla3 <- pwOrthReg(data = zf.padilla.df3, group = "group", limit.comp = "ZF_EMBRYO1", min.n = 5, message = T, plot = T)
or.zf.padilla.df3 <- data.table::rbindlist(or.zf.padilla3$orthregs)
