...
#===========================================================#
# Get AMPHIBIANS, prioritize 96 hr LC50
#===========================================================#
# OECD test 201 is 3day biomass https://www.oecd.org/en/publications/test-no-201-alga-growth-inhibition-test_9789264069923-en.html
AMPH <- dat %>%
  dplyr::filter(source %in% c("Widmayer et al. 2022", "Boyd et al. 2016", "EnviroTox DB")) %>%
  dplyr::filter(group == "NEMATODE" | group == "AMPHIB" | group == "NEMATODE_COPAS1") %>%
  dplyr::filter(case_when(group == "NEMATODE" ~ T,
                          group == "NEMATODE_COPAS1" ~ T,
                          source == "EnviroTox DB" & test_statistic == "LC50" & endpoint == "Mortality" ~ T,  #
                          TRUE ~ F))

# run the orth regressions across all pairs without QC filter
AMPHreg <- pwOrthReg(data = AMPH, group = "group", limit.comp = "NEMATODE", min.n = 5, message = F, QC = "ignore", plot = T)
AMPHreg_df <- data.table::rbindlist(AMPHreg$orthregs) %>%
  cbind(data.table::rbindlist(AMPHreg$normRegs)[,3:9]) %>%
  dplyr::arrange(desc(orth.reg.n.observations)) %>%
  dplyr::mutate(or.slope = round(orth.reg.slope, digits = 3),
                or.int = round(orth.reg.intercept, digits = 3)) %>%
  dplyr::select(x, y,
...
mammal <- dat %>%
  ...
  dplyr::filter(group == "NEMATODE" | group == "NEMATODE_COPAS1" | group == "NIEHS_RAT" | group == "NIEHS_RABBIT" | group == "NIEHS_MOUSE") %>%
  dplyr::filter(case_when(group == "NEMATODE" ~ T,
                          group == "NEMATODE_COPAS1" ~ T,
                          source == "NIEHS_ICE" ~ T, # removing EC50s has a big filtering effect, even though some are coded as mortality
                          TRUE ~ F)) 
...
tab2_df <- ALLreg_df %>%
  dplyr::mutate(keep = dplyr::case_when(x == "NEMATODE_COPAS1:AC50:NA:Growth" & y == "FISH:LC50:4:Mortality" ~ T,
                                        ...
                                        x == "NEMATODE_COPAS1:AC50:NA:Growth" & y == "AMPHIB:LC50:4:Mortality" ~ T,
                                        ...
                                        x == "NEMATODE:EC10:2:Growth" & y == "AMPHIB:LC50:4:Mortality" ~ T,
                                        x == "FISH:LC50:4:Mortality" & y == "ALGAE:EC50:4:Population" ~ T,
                                        x == "FISH:LC50:4:Mortality" & y == "AMPHIB:LC50:4:Mortality" ~ T,
                                        x == "FISH:LC50:4:Mortality" & y == "INVERT:LC50:2:Mortality" ~ T,
                                        x == "INVERT:LC50:2:Mortality" & y == "ALGAE:EC50:4:Population" ~ T,
                                        x == "INVERT:LC50:2:Mortality" & y == "AMPHIB:LC50:4:Mortality" ~ T,
                                        x == "ALGAE:EC50:4:Population" & y == "AMPHIB:LC50:4:Mortality" ~ T,
                                        TRUE ~ F)) %>%
  dplyr::mutate(`Species group 1` = case_when(x == "NEMATODE_COPAS1:AC50:NA:Growth" ~ "Nematode COPAS",
                                              ...
                                              x == "INVERT:LC50:2:Mortality" ~ "INVERT",
                                              x == "ALGAE:EC50:4:Population" ~ "ALGAE",
                                              x == "AMPHIB:LC50:4:Mortality" ~ "AMPHIBIAN"),
                `Species group 2` = case_when(y == "NEMATODE_COPAS1:AC50:NA:Growth" ~ "Nematode COPAS",
                                              ...
                                              y == "INVERT:LC50:2:Mortality" ~ "INVERT",
                                              y == "ALGAE:EC50:4:Population" ~ "ALGAE",
                                              y == "AMPHIB:LC50:4:Mortality" ~ "AMPHIBIAN")) %>%
  dplyr::filter(keep == T) %>%
  dplyr::arrange(desc(`Species group 1`), `Species group 2`) %>%
  dplyr::select(`Species group 1`, `Species group 2`, or.slope:`Deg. freedom`)

# output the table
rio::export(tab2_df, file = glue::glue("data/processed/{today}_table2_mean_not_gm_mean.csv"))


#===========================================================#
# Join groups and do full pairwise
#===========================================================#
ALL <- dplyr::bind_rows(FISH, INVERT, ALGAE, AMPH) %>%
